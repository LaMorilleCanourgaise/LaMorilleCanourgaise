...
const recipeMenuLanguage = document.documentElement.lang;
const recipeMenu = recipeMenuLanguage === "en"
  ? { label: "Recipe ideas", href: "recettes.html" }
  : recipeMenuLanguage === "es"
    ? { label: "Ideas de recetas", href: "recetas.html" }
    : { label: "Idées de recettes", href: "recettes.html" };
document.querySelectorAll(".nav-links").forEach((menu) => {
  if (menu.querySelector('a[href="recettes.html"], a[href="recetas.html"]')) return;
  const historyLink = Array.from(menu.querySelectorAll("a")).find((link) => /histoire|history|historia/i.test(link.textContent));
  if (!historyLink) return;
  const item = document.createElement("li");
  const link = document.createElement("a");
  link.href = recipeMenu.href;
  link.textContent = recipeMenu.label;
  if (/(^|\/)(recettes|recetas)\.html$/i.test(window.location.pathname)) link.classList.add("active");
  item.append(link);
  historyLink.closest("li")?.after(item);
});
document.querySelectorAll(".nav-links").forEach((menu) => {
  if (menu.querySelector('a[href$="/avis.html"], a[href$="/reviews.html"], a[href$="/opiniones.html"], a[href="avis.html"], a[href="reviews.html"], a[href="opiniones.html"]')) return;
  const language = document.documentElement.lang;
  const contactLink = Array.from(menu.querySelectorAll("a")).find((link) => /contact|contacto/i.test(link.textContent));
  if (!contactLink) return;
  const item = document.createElement("li");
  const link = document.createElement("a");
  link.href = language === "en" ? "avis.html" : language === "es" ? "avis.html" : "avis.html";
  link.textContent = language === "en" ? "Reviews" : language === "es" ? "Opiniones" : "Avis";
  if (/\/(?:avis|reviews|opiniones)\.html$/i.test(window.location.pathname)) link.classList.add("active");
  item.append(link);
  const accountLink = Array.from(menu.querySelectorAll("a")).find((link) => /mon compte|my account|mi cuenta|se connecter|log in|iniciar sesión/i.test(link.textContent));
  if (accountLink?.closest("li")) {
    accountLink.closest("li").before(item);
  } else {
    contactLink.closest("li")?.after(item);
  }
});
document.querySelectorAll(".recipe-card-image").forEach((image) => {
  image.addEventListener("click", () => {
    const target = image.closest(".recipe-card")?.querySelector(".button[href]");
...
const recipeDetailImageMap = {
  "recette-aligot-maroilles.html": "photos/detail-aligot-maroilles.jpg",
  "recette-filet-mignon-veau-morilles.html": "photos/detail-filet-mignon-veau-morilles.jpg",
  "recette-supreme-pintade-morilles-vin-jaune.html": "photos/detail-supreme-pintade-morilles.webp",
  "recette-omelette-morilles.html": "photos/detail-omelette-morilles.webp",
  "recette-grenadins-veau-morilles.html": "photos/detail-grenadins-veau-morilles.jpg",
  "recette-morilles-vin-jaune-creme.html": "photos/detail-morilles-vin-jaune.webp",
  "recette-feuillete-asperges-morilles.html": "photos/detail-feuillete-asperges-morilles.webp",
  "recette-gratin-lasagnes-morilles-parmesan.html": "photos/detail-gratin-lasagnes-morilles.webp",
  "recette-croutes-morilles.html": "photos/detail-croutes-morilles.jpg",
  "recette-noix-saint-jacques-morilles.html": "photos/detail-noix-saint-jacques-morilles.webp",
  "recette-mille-feuille-pommes-terre-morilles-truffe.html": "photos/detail-mille-feuille-pommes-terre-morilles-truffe.webp",
  "recette-boeuf-creme-morilles.html": "photos/detail-boeuf-creme-morilles.webp",
  "recette-poularde-morilles-cocotte.html": "photos/detail-poularde-morilles-cocotte.webp",
  "recette-asperges-creme-morilles.html": "photos/detail-asperges-creme-morilles.webp",
  "recette-beignets-pommes-terre-morilles.html": "photos/detail-beignets-pommes-terre-morilles.webp",
  "recette-morilles-creme.html": "photos/morillealacreme.png",
  "recette-risotto-morilles.html": "photos/risottoauxmorilles.jpg",
  "recette-oeufs-morilles.html": "photos/oeufscocottemorilles.jpg",
  "recette-poisson-crunchy-saveurs-boisees.html": "photos/detail-poisson-crunchy-saveurs-boisees.jpg",
  "recette-poulet-fermier-morilles-vin-jaune.html": "photos/detail-poulet-fermier-morilles-vin-jaune.webp",
  "recette-quiche-morilles.html": "photos/detail-quiche-morilles.jpg"
};

const applyRecipeDetailImage = () => {
  const currentPage = (window.location.pathname.split("/").pop() || "").toLowerCase();
  const selectedKey = currentPage.replace(/^receta-/, "recette-");
  const asset = recipeDetailImageMap[selectedKey];
  if (!asset) return;
  const resolvedAsset = /\/lang\/(?:en|es)\//i.test(window.location.pathname) ? `../../${asset}` : asset;
  document.querySelectorAll(".recipe-detail-image, .recipe-detail-photo").forEach((element) => {
    const img = element.querySelector("img");
    if (img) {
      img.src = `${resolvedAsset}?v=${Date.now()}`;
      img.alt = img.alt || element.getAttribute("aria-label") || "Recette morilles";
      return;
    }
    element.style.backgroundImage = `url("${resolvedAsset}?v=${Date.now()}")`;
    element.style.backgroundSize = "cover";
    element.style.backgroundPosition = "center";
  });
};

applyRecipeDetailImage();
if (toggle && links) {
...
const reviewsStorageKey = "la-morille-canourgaise-reviews";
const reviewsResetKey = "la-morille-canourgaise-reviews-reset";
const reviewAvatarColors = ["#3e6048", "#c58a42", "#7c5b8f", "#9b5c52", "#527b91", "#788c4a", "#b56f82"];
if (localStorage.getItem(reviewsResetKey) !== "5") {
  localStorage.removeItem(reviewsStorageKey);
  localStorage.setItem(reviewsResetKey, "5");
}

const loadReviews = () => {
  try {
    const saved = JSON.parse(localStorage.getItem(reviewsStorageKey) || "[]");
    return Array.isArray(saved) ? saved.filter((review) => review && review.name && review.message) : [];
  } catch {
    return [];
  }
};
...
const renderReviews = () => {
  ...
  reviews.forEach((review, index) => {
    ...
    avatar.style.background = review.color || reviewAvatarColors[index % reviewAvatarColors.length];
    ...
    stars.setAttribute("aria-label", `${review.rating} sur 5`);
  ...
  if (reviewsEmpty) reviewsEmpty.hidden = reviews.length > 0;
  if (reviewsCount) reviewsCount.textContent = String(reviews.length);
  if (reviewsBreakdown) {
    reviewsBreakdown.replaceChildren();
    for (let rating = 5; rating >= 1; rating -= 1) {
      ...
      track.className = "review-breakdown-track";
      const fill = document.createElement("span");
      fill.className = "review-breakdown-fill";
      fill.style.width = reviews.length ? `${(count / reviews.length) * 100}%` : "0%";
      track.append(fill);
      const total = document.createElement("strong");
      total.textContent = `${count} ${count === 1 ? "vote" : "votes"}`;
      row.append(label, track, total);
      reviewsBreakdown.append(row);
    }
  }
  if (reviewsAverage) {
    const average = reviews.length
      ...
      : 0;
    reviewsAverage.textContent = average ? `${average.toFixed(1).replace(".", ",")} / 5` : "— / 5";
    const averageStars = document.querySelector("#reviews-average-stars");
    if (averageStars) {
...
if (reviewsForm) {
  ...
  reviewsForm.addEventListener("submit", (event) => {
    event.preventDefault();
    const data = Object.fromEntries(new FormData(reviewsForm));
    ...
    reviews.unshift({
      ...
      rating,
      color: reviewAvatarColors[reviews.length % reviewAvatarColors.length],
      date: new Intl.DateTimeFormat(document.documentElement.lang || "fr", { dateStyle: "long" }).format(new Date())
    });
    localStorage.setItem(reviewsStorageKey, JSON.stringify(reviews));
    reviewsForm.reset();
    reviewsForm.querySelector('[name="rating"]').value = "5";
    renderReviews();
...
const translations = {
  en: {
    ...
    "Au plaisir de vous accompagner": "We are happy to help", "Parlons de vos envies": "Tell us what you need", "Une question ? Parlons-en →": "Have a question? Let's talk →", "Avis": "Reviews"
  },
  es: {
    ...
    "Échangeons": "Hablemos", "Se connecter": "Iniciar sesión", "Créer un compte": "Crear una cuenta", "Avis": "Opiniones",
...
const existingLanguageSwitchers = Array.from(document.querySelectorAll(".language-switcher"));
const languageSwitcher = existingLanguageSwitchers[0] || document.createElement("div");
existingLanguageSwitchers.slice(1).forEach((switcher) => switcher.remove());
...
let pageForLanguage = currentPage;
if (currentPage === "recettes.html" || currentPage === "recetas.html") {
  pageForLanguage = "recettes.html";
} else if (/^receta-/.test(currentPage)) {
  pageForLanguage = currentPage.replace(/^receta-/, "recette-");
}
Object.entries(languageLabels).forEach(([language, label]) => {
  const link = document.createElement("a");
  const targetPage = language === "es" && /^recette-/.test(pageForLanguage)
    ? pageForLanguage.replace(/^recette-/, "receta-")
    : language === "es" && pageForLanguage === "recettes.html"
      ? "recetas.html"
      : pageForLanguage;
  link.href = languagePathMatch
    ? language === "fr"
      ? `../../${targetPage}`
      : `../${language}/${targetPage}`
    : language === "fr"
      ? targetPage
      : `lang/${language}/${targetPage}`;
  link.dataset.language = language;
  link.textContent = label;
  languageSwitcher.append(link);
});
const pageNav = document.querySelector("nav");
if (pageNav) {
  if (languageSwitcher.parentElement !== pageNav) {
    pageNav.append(languageSwitcher);
  }
  languageSwitcher.classList.remove("language-switcher-floating");
} else {
  languageSwitcher.classList.add("language-switcher-floating");
...
const applyLanguage = (language) => {
  ...
  document.querySelectorAll(".language-switcher").forEach((switcher) => {
    switcher.querySelectorAll("a[data-language]").forEach((link) => {
